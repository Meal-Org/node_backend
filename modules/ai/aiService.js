...
const { getRecipeByIngredient } = require('../recipe_management/recipeService');
  ...
  try {
    switch (intent) {
      ...
      case "recipe":
        ...
        responseContent = entities.length > 0 ?
          await getRecipeByIngredient(entities[0]) :
    ...
    await prisma.interaction.create({
      data: {
        ...
        // Ensure userId is either a valid ID or explicitly set to null
      },
    });
    ...
    throw new OpenAIError('Failed to process the request.');
  }
};
